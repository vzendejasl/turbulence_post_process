...
usage() {
  cat <<'EOF'
Usage:
  sbatch scripts/run_dane_postprocess.sh <sampled_data_root> [<sampled_data_root> ...]

Example:
  sbatch scripts/run_dane_postprocess.sh \
...
Notes:
  ...
  - override node/task/time options at submit time as needed, for example:
      sbatch --nodes=2 --ntasks-per-node=112 --time=04:00:00 scripts/run_dane_postprocess.sh ...
EOF
}
...
POSTPROC_RANKS="${POSTPROC_RANKS:-${SLURM_NTASKS:-112}}"
...
POSTPROC_CONDA_ENV="${POSTPROC_CONDA_ENV:-heffte-py-dane}"
POSTPROC_REPO="${POSTPROC_REPO:-/g/g11/zendejas/turbulence_post_process}"
POSTPROC_MAIN="${POSTPROC_MAIN:-$POSTPROC_REPO/main.py}"
POSTPROC_HEFFTE_PY="${POSTPROC_HEFFTE_PY:-/p/lustre1/zendejas/third_party/heffte/install/share/heffte/python}"
POSTPROC_HEFFTE_LIB="${POSTPROC_HEFFTE_LIB:-/p/lustre1/zendejas/third_party/heffte/install/lib64}"
POSTPROC_MPI_LIB="${POSTPROC_MPI_LIB:-/usr/tce/packages/mvapich2/mvapich2-2.3.7-gcc-13.3.1/lib}"
...
export LD_LIBRARY_PATH="${POSTPROC_HEFFTE_LIB}:${POSTPROC_MPI_LIB}:${LD_LIBRARY_PATH:-}"
